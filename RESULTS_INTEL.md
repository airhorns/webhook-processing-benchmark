...
- Elixir — Bandit + Plug + Jason on OTP 28 / Elixir 1.19.5.
...
| Elixir | 1 | OTP 28 `:json` | 8,346 | +6.9% | 30,671 | 38,143 | 126.4% | +7.1% | CAP FAIL |
| Elixir | 4 | OTP 28 `:json` | 29,413 | +7.3% | 8,527 | 12,663 | 474.9% | -2.0% | CAP FAIL |
...
- Elixir — selected OTP 28 built-in `:json` module (`WPB_JSON=otp28`).
...
| Result | rps | Reason |
|--------|----:|--------|
| `elixir-1c-baseline` | 7,808 | cap_holds=false (125.8% avg vs 100% cap) |
| `elixir-1c-otp28-json` | 8,346 | cap_holds=false (126.4% avg vs 100% cap) |
| `elixir-4c-baseline` | 27,421 | cap_holds=false (474.0% avg vs 400% cap) |
| `elixir-4c-otp28-json` | 29,413 | cap_holds=false (474.9% avg vs 400% cap) |

Investigation: the failing set is limited to Elixir. The harness launched BEAM with the intended scheduler cap flags (`+S N:N +SDcpu 1:1 +SDio 1 +A 1 +sbwt none +sbwtdcpu none +sbwtdio none`), errors stayed at zero, and the client was not bottlenecked. CPU sampling still measured ~126% at 1c and ~474–475% at 4c. This is materially worse drift than the M4 Pro note (~10–13%) and should be considered a platform/runtime cap limitation for these Elixir numbers on this host unless the harness is changed and re-run.

## Notable observations vs Apple M4 Pro

- Baseline 1c ranking on this host: Rust (27,657) > Elixir (7,808) > Go (6,772) > Ruby (5,346).
- Baseline 4c ranking on this host: Rust (90,313) > Elixir (27,421) > Ruby (22,680) > Go (19,065).
...
- Elixir absolute throughput roughly matches the M4 Pro at 1c and is slightly behind at 4c, but every Elixir number is validity-flagged because BEAM CPU usage exceeded the cap by ~26% at 1c and ~19% at 4c.
...
WPB_JSON=otp28 bench/run.sh elixir 1 otp28-json "OTP 28 :json module"
